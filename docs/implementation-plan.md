...
- `uv run --locked pytest tests/test_inventory.py -q` — 100 passed。
...
- `make ci-check` — 成功。
- `make test` — 1027 passed, 2 skipped。
- `uv lock --check` — 成功。
- `git diff --check` — 成功。

性能実測:

- 変更前はユーザー実測13.032秒（過去レビュー12.56秒）だったのに対し、変更後はisolatedな空の`UV_CACHE_DIR`で初回7.10秒、同一cacheの2回目4.31秒だった。
- 初回実行にも効果があるscan-local最適化であり、実行間で再利用する永続的なanalysis cacheではない。
