...
- distribution scanごとに`physical_prefix.resolve(strict=False)`を一度だけ取得し、`_physical_path`と`_parent_is_safe`へ共有した。各componentの`current.resolve()`、parent safetyの`path.parent.resolve()`、最終`lstat()`は維持した。
