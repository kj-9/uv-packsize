...
| 項目 | 状態 |
|---|---|
| 現在のPhase | Follow-up: inventory path resolution performance |
| `in_progress` | なし |
...
- `pyproject.toml`、`uv.lock`、artifact verifier、release metadata testsのversionを`0.2.1`へ同期する。
- `422b159`、`02df817`、`c5d2403`、`a861308`で実装したinventory性能改善をpatch releaseの記録へ反映する。
- release/tag/pushは実施せず、lock、関連tests、metadata整合性を検証する。

変更:

- `pyproject.toml`、`uv.lock` root package、`scripts/verify_build.py`、
  `tests/test_uv_packsize.py`、`tests/test_verify_build.py`を`0.2.1`へ同期した。
- `422b159`はgenerated bytecode cacheのindex、`02df817`はinventory path lookupの再利用、
  `c5d2403`はresolved inventory pathの再利用、`a861308`はscan-localなpath resolutionの
  memoizeを実装した。case ambiguity、中間symlink、prefix containment、filesystem warning、
  logical sizeの公開契約は維持する。
- 同一条件の`llm`測定では、変更前後のwarm wall timeが約1.83〜1.84秒から
  約1.10〜1.11秒へ短縮した。TOCTOU修正後の同条件warm再実行は1.47秒だった。
  今回のrelease記録は、これら4コミットと安全性修正を含む性能改善を`0.2.1`のpatch
  scopeとして固定する。
- レビューで検出したTOCTOU回帰を修正した。`_parent_is_safe()`のcontainment検査は
  scan-localなpath resolution cacheを使わず、`lstat()`直前に親directoryを独立して
  `resolve()`する。スキャン中に親directoryをprefix外symlinkへ置換する回帰testも追加した。

検証:

- `uv lock` — 成功（root packageを`0.2.0`から`0.2.1`へ更新）。
- `uv run --locked pytest tests/test_inventory.py tests/test_uv_packsize.py tests/test_verify_build.py -q`
  — 281 passed。
- `UV_CACHE_DIR=/private/tmp/uv-packsize-v021-ci-cache make ci-check` — 成功。
- `UV_CACHE_DIR=/private/tmp/uv-packsize-v021-test-cache make test` — 1029 passed, 2 skipped。
- `UV_CACHE_DIR=/private/tmp/uv-packsize-v021-lock-cache uv lock --check`、`git diff --check` — 成功。
- 一意なtemporary output directoryで`uv build --no-sources --out-dir <temporary-directory>`と
  `uv run --locked python scripts/verify_build.py <temporary-directory>` — `0.2.1` wheel/sdistの
  metadata、archive、installed entry pointを検証。

制約:

- 既存`dist/`には過去版artifactが残っているため、`make verify-build`の直接実行は複数版検出で
  失敗する。過去artifactを削除せず、一意なtemporary output directoryで同じbuild/verifierを
  実行して検証した。
- release、Git tag、push、PyPI publishはこの準備では実施していない。

追補検証（2026-09-13）:

- `UV_CACHE_DIR=/private/tmp/uv-packsize-review-cache uv run --locked pytest tests/test_inventory.py -q`
  — 103 passed。
- `UV_CACHE_DIR=/private/tmp/uv-packsize-review-cache uv run --locked pytest -q`
  — 1030 passed, 2 skipped。
- 対象2ファイルのRuff check/format check、`uv lock --check`、`git diff --check` — 成功。
- `uv run --no-sync uv-packsize llm --quiet --json`（専用cache、macOS） — 初回4.41秒、
  warm再実行1.47秒。いずれも29 distributions、3,269 files、global logical bytes
  28,507,915で、出力JSONの構造と件数を確認した。

### 2026-09-13: Inventory path resolution performance

状態: `done`

対象:

- `inventory.py` のcase-insensitive directory listing再利用と、generated bytecode candidateの重複path resolution削減を行う。
- 中間symlink/prefix containment検査、最終`lstat()`、case ambiguity、filesystem warningの公開契約を維持する。

完了条件:

- scan-local indexとbytecode candidate変更に対応する性能・安全性・warning回帰テストがある。
- 対象test、Ruff check/format、全体checkを実行し、結果をここへ記録する。

今回の回帰対応:

- case-insensitive hostでcase-only filenameを作れない場合でもambiguityを検証できるよう、テストのdirectory listingを対象platformのfixtureとして注入した。
- scan-local directory indexの検査失敗は各RECORD rowごとに内部warningを生成することと、`DistributionResult`が維持する同一warningの公開dedup（2 rowでも1件）を別々に検証した。
- directory listingのindex処理をhelperへ分離し、Ruffの分岐数制限を満たしながら、中間symlinkを列挙しないprefix containment検査を維持した。
- distribution scanごとに`physical_prefix.resolve(strict=False)`を一度だけ取得し、`_physical_path`へ共有した。各componentの探索はscan-local cacheを再利用する一方、parent safetyの`path.parent.resolve()`は`lstat()`直前に独立実行し、containmentの再検査を維持した。
- RECORDで成功したsourceの`ResolvedRecordPath`をgenerated bytecode探索へ渡し、同じsourceの再解決を除去した。direct/cache childは既存のcase-insensitive directory indexで解決し、ambiguity、cache symlink、prefix containment、warning処理を維持した。

検証:

- `uv run --locked pytest tests/test_inventory.py -q` — 101 passed。
- `uv run --locked ruff check uv_packsize/inventory.py tests/test_inventory.py` — 成功。
- `uv run --locked ruff format --check uv_packsize/inventory.py tests/test_inventory.py` — 成功。
- `make ci-check` — 成功。
- `make test` — 1028 passed, 2 skipped。
- `uv lock --check` — 成功。
- `git diff --check` — 成功。

性能実測:

- ユーザー目的はcacheなし初回の製品CLI計算時間短縮である。計測基準は、開発リポジトリのbootstrapを含めない既存dev environmentから、`uv run --no-sync uv-packsize llm`を起動し、実行ごとに新しい空の`UV_CACHE_DIR`を与える条件とする。
- 最新実装の独立したcold実測は3.97秒、3.44秒だった。いずれも29 distributions、3,269 filesを検出し、inventoryのdistribution/file/total値とrich projectionの内容が一致した。
- 同一条件でcacheを再利用するwarm実測は約1.9〜2.0秒だった。最適化はtemporary scan内だけで完結し、永続的なanalysis cacheやambient設定は導入していない。
- `UV_PROJECT_ENVIRONMENT`も新規にしてbootstrapまで含めた`uv run uv-packsize llm`は14.11秒だったが、そのうち6.86〜8.19秒は開発依存36 package（約45.6 MiB）のproject同期・起動である。この値は製品CLIのcold計算基準ではなく、開発リポジトリbootstrapコストとして分離して記録する。
