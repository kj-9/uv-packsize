...
`0.2.1`は`0.2.0`の互換性を維持するpatch releaseとして、inventory path
resolutionの性能改善を含める。`422b159`はgenerated bytecode cacheのindexを導入し、
`02df817`はinventory path lookupを再利用し、`c5d2403`はresolved inventory pathを
再利用し、`a861308`はscan-localなpath resolutionをmemoizeする。これらは
case ambiguity、中間symlink、prefix containment、filesystem warning、logical size
の公開契約を維持したまま、同じinventory結果をより少ないfilesystem操作で生成する。

project/artifact metadataと`uv.lock`のroot versionを`0.2.1`へ同期し、性能改善と検証結果を
実装計画へ記録した。`v0.2.1`はcommit `c78a06c`を基準にGitHub Release（non-draft、
non-prerelease）として2026-09-13に公開済みである。publish workflow
`34739507919`はverify/test（Python 3.10〜3.14）/deploy/PyPIの全工程に成功し、
PyPI上でのexact version指定とfresh latest selection（いずれも`0.2.1`）も確認した。
