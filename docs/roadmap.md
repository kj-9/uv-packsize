...
この準備ではproject/artifact metadataと`uv.lock`のroot versionを`0.2.1`へ同期し、
性能改善と検証結果を実装計画へ記録する。GitHub release、tag、push、PyPI publishは
リリース担当者が明示的に実施するため、この作業には含めない。
