...
def _case_insensitive_child(
    ...
    """Resolve one child using an already trusted physical parent."""
...
def _scan_bytecode_cache(cache_dir: Path) -> tuple[Path, ...]:
    """Index the bytecode files in one cache directory exactly once."""
...
def _bytecode_candidates(  # noqa: PLR0913
    layout: InventoryLayout,
    source: FileEntry,
    *,
    ...
    if source_resolved is None:
        ...
        direct_candidate = source_parent / f"{source_path.stem}.pyc"
        cache_dir = source_parent / "__pycache__"
    else:
        direct_candidate = _case_insensitive_child(
            source_parent,
            f"{source_path.stem}.pyc",
            directory_index,
        )
        cache_dir = _case_insensitive_child(
            source_parent,
            "__pycache__",
            directory_index,
        )
    ...
    if indexed is None:
        try:
            indexed = _scan_bytecode_cache(cache_dir)
        except (RecordPathOutsidePrefixError, FilesystemInventoryError) as error:
            cache_index[cache_dir] = error
            raise
        cache_index[cache_dir] = indexed
    if isinstance(indexed, Exception):
        raise indexed
...
def _generated_bytecode(  # noqa: PLR0913
    *,
    layout: InventoryLayout,
    ...
    for source in record_entries:
        if not source.path.casefold().endswith(".py"):
            continue
        try:
            candidates = _bytecode_candidates(
                layout,
                source,
                cache_index=cache_index,
                directory_index=directory_index,
                source_resolved=(
                    resolved_record_paths.get(source.canonical_identity)
                    if resolved_record_paths is not None
                    else None
                ),
...
def _collect_record_entries(  # noqa: PLR0913
    *,
    layout: InventoryLayout,
    dist_info_dir: Path,
    ...
    resolved_record_paths: dict[str, ResolvedRecordPath] = {}
    for recorded_path in recorded_paths:
        ...
        if entry is None:
            ...
            entries.append(entry)
            resolved_record_paths[resolved.canonical_identity] = resolved
...
def collect_distribution(  # noqa: PLR0911
    *,
    layout: InventoryLayout,
    dist_info_dir: Path,
    resolved_path_index: ResolvedPathIndex | None = None,
